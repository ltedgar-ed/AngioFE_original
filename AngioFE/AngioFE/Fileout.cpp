...
void Fileout::writeData(FEAngio &feangio)
{
	...
	for (it = feangio.frag.begin(); it != feangio.frag.end(); ++it)                         // Iterate through all segments in frag list container (it)
	{
		//fprintf(stream,"%-12.7f %-12.7f %-12.7f %-12.7f %-12.7f %-12.7f %-12.7f %-12.7f %-5.2i %-5.2i\n",it->TofBirth,it->x[0],it->y[0],it->z[0],it->x[1],it->y[1],it->z[1],it->length,it->vessel,it->label);  // Write to data.ang
...
void Fileout::writeCollFib(Grid &grid, bool initial)
{
	...
	for (int i = 0; i < grid.Nn; ++i)
	{
		fprintf(node_stream,"%-5.2i %-12.7f %-12.7f %-12.7f %-12.7f %-12.7f %-12.7f\n", grid.nodes[i].id, grid.nodes[i].x, grid.nodes[i].y, grid.nodes[i].z, grid.nodes[i].collfib.x, grid.nodes[i].collfib.y, grid.nodes[i].collfib.z);
	}
...
void Fileout::writeECMDen(Grid &grid)
{
	...
	for (int i = 0; i < grid.Nn; ++i)
	{
		fprintf(node_stream,"%-5.2i %-12.7f %-12.7f %-12.7f %-12.7f %-12.7f\n", grid.nodes[i].id, grid.nodes[i].x, grid.nodes[i].y, grid.nodes[i].z, grid.nodes[i].ecm_den, grid.nodes[i].ecm_den0);
	}
...
void Fileout::writeECMDenGrad(Grid &grid)
{
	...
	for (int i = 0; i < grid.Nn; ++i)
	{
		fprintf(node_stream,"%-5.2i %-12.7f %-12.7f %-12.7f %-12.7f %-12.7f %-12.7f\n", grid.nodes[i].id, grid.nodes[i].x, grid.nodes[i].y, grid.nodes[i].z, grid.nodes[i].ecm_den_grad.x, grid.nodes[i].ecm_den_grad.y, grid.nodes[i].ecm_den_grad.z);
	}
